...
type PanelTab = "images" | "labels" | "theme";
...
export default function EditorPage() {
  ...
  const renderImagesPanel = (compact = false) => (
    <>
      <div className={`flex-shrink-0 px-4 ${compact ? "pt-0 pb-2" : "pt-4 pb-2"} flex items-center gap-2`}>
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-xs text-zinc-200 font-mono tabular-nums">
                {present.before.transform.zoom.toFixed(2)}x
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) => {
                ...
                commit((prev) => ({
                  ...
                  before: {
                    ...prev.before,
                    transform: { ...prev.before.transform, zoom },
                  },
                }));
              }}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-xs text-zinc-200 font-mono tabular-nums">
                {present.after.transform.zoom.toFixed(2)}x
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) => {
                ...
                commit((prev) => ({
                  ...
                  after: {
                    ...prev.after,
                    transform: { ...prev.after.transform, zoom },
                  },
                }));
              }}
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[#0d0d0f] text-zinc-50 flex flex-col overflow-y-auto lg:overflow-hidden">
      {toastMessage && (
      ...
      <header className="flex-shrink-0 min-h-14 border-b border-white/[0.06] bg-[#0d0d0f]/95 backdrop-blur-xl flex flex-wrap items-center px-3 sm:px-4 py-2 sm:py-0 gap-2 sm:gap-3 z-10">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-visible lg:overflow-hidden min-h-0">
        <aside className="hidden lg:flex w-[268px] flex-shrink-0 border-r border-white/[0.06] flex-col overflow-hidden bg-[#111113]">
          {renderImagesPanel()}
        </aside>

        <main className="order-1 lg:order-none flex-1 min-w-0 min-h-[52vh] lg:min-h-0 flex items-center justify-center overflow-visible lg:overflow-auto bg-[#0d0d0f] relative">
          <div
          ...
          <div
            ...
            className="relative z-10 w-full lg:h-full min-h-[52vh] lg:min-h-0 flex items-center justify-center p-3 sm:p-6"
          >
            <EditorCanvas
              editor={present}
              ...
              onObjectFitComputed={(slotId, fit) => {
                setPresent((prev) => ({
                  ...
                  [slotId]: {
                    ...prev[slotId],
                    transform: { ...prev[slotId].transform, objectFit: fit },
                  },
                }));
              }}
        ...
        <aside className="hidden lg:flex w-[268px] flex-shrink-0 border-l border-white/[0.06] flex-col overflow-hidden bg-[#111113]">
          <div className="flex-shrink-0 flex border-b border-white/[0.06]">
            {([
              {
                id: "labels" as PanelTab,
                label: "Labels",
                icon: <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M1.5 9.5l5-5 2 2-5 5H1.5v-2zm6-6l1.5-1.5a1 1 0 011.5 1.5L9 5l-1.5-1.5z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" /></svg>,
              },
              {
                id: "theme" as PanelTab,
                label: "Theme",
                icon: <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.2" /><path d="M6.5 1.5C6.5 1.5 4 4 4 6.5s2.5 5 2.5 5" stroke="currentColor" strokeWidth="1.2" /><path d="M1.5 6.5h10" stroke="currentColor" strokeWidth="1.2" /></svg>,
              },
            ] as const).map((tab) => (
              <button
                key={tab.id}
          ...
          <div
            ...
            style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.1) transparent" }}
          >
            {activeTab === "labels" && renderLabelsPanel()}
            {activeTab === "theme" && renderThemePanel()}
          </div>
        </aside>

        <section className="order-2 lg:hidden flex-shrink-0 border-t border-white/[0.06] bg-[#111113] flex flex-col">
          <div className="grid grid-cols-3 border-b border-white/[0.06]">
            {([
              { id: "images" as PanelTab, label: "Images" },
              { id: "labels" as PanelTab, label: "Labels" },
              { id: "theme" as PanelTab, label: "Theme" },
            ] as const).map((tab) => (
              <button
                key={tab.id}
                type="button"
                onClick={() => setActiveTab(tab.id)}
                className={[
                  "py-3 text-xs font-medium border-b-2 transition-all",
                  activeTab === tab.id
                    ? "text-zinc-100 border-violet-500 bg-violet-500/5"
          ...
          <div className="px-3 py-3 space-y-3">
            {activeTab === "images" && renderImagesPanel(true)}
            {activeTab === "labels" && renderLabelsPanel()}
            {activeTab === "theme" && renderThemePanel()}
          </div>
        </section>
